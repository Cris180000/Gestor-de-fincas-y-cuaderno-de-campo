...
export function validateFinca(
  body: unknown
): { ok: true; data: { nombre: string; ubicacion?: string; superficie?: number; notas?: string; referenciaCatastral?: string } } | { ok: false; errors: string[] } {
  const errors: string[] = [];
  ...
  const ubicacion = typeof b.ubicacion === "string" ? b.ubicacion.trim() || undefined : undefined;
  ...
  return { ok: true, data: { nombre, ubicacion, superficie, notas, referenciaCatastral } };
}
